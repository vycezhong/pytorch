...
def handler(signum, frame):
    """Failure simulation handler.

    We simulate the failure by sending the SIGTERM signal to the target agent process.
    After receiving the signal, the agent will stop all the workers on the machine,
    sleep for a while to simulate the down time, and restart itself.
    """
    logger.info(f"The agent has received signal {signum} and will stop workers.")
    # Get the agent.
    ...
    # NB: simulate the node being down (default: 15s).
    time.sleep(15)

    logger.info("The agent restarts.")
    # A tricky way to restart the process.
...
def launch_agent(
    ...
    # NB: we use the SIGTERM signal to indicate the failure.
